...
export function PoolDetailPage() {
  ...
  const investLabel = detail.estado !== "abierto" ? "Pool no disponible" : profile?.rol !== "inversionista" ? "Solo para inversionistas" : !profile.wallet_public_key ? "Wallet en preparación" : "Realizar aporte";
    ...
    <div className="page-stack">
      ...
      <section className="detail-layout">
        <div className="detail-main">
          <section className="section-stack"><div className="section-heading"><div><p className="eyebrow">ELIGE TU PERFIL</p><h2>Compara los tramos</h2></div></div><div className="tranche-grid">{(["senior", "junior"] as TrancheType[]).map((tranche) => <TrancheCard key={tranche} type={tranche} detail={detail} selected={selectedTranche === tranche} onSelect={() => setSelectedTranche(tranche)} />)}</div><div className="mobile-investment-cta"><div><span>Tramo {trancheLabel(selectedTranche)}</span><strong>{detail.tramos[selectedTranche].rendimiento_ilustrativo_plazo_pct}% ilustrativo</strong></div><Button onClick={() => setInvestOpen(true)} disabled={!canInvest}>{investLabel}{canInvest && <ArrowRight size={17} />}</Button></div></section>
...
function InvestmentModal({ open, onClose, detail, initialTranche, session, onSuccess }: { open: boolean; onClose: () => void; detail: PoolDetail; initialTranche: TrancheType; session: ReturnType<typeof useAuth>["session"]; onSuccess: () => void }) {
  ...
  return <Modal className={`investment-modal investment-modal--${step}`} open={open} title={title} description={step === "success" ? "Tu posición ya forma parte del portafolio." : detail.nombre} onClose={resetAndClose} dismissible={step !== "processing"}>
