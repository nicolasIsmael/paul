...
function TopUpModal({ open, session, onClose, onSuccess }: { open: boolean; session: Session | null; onClose: () => void; onSuccess: (result: TopUpResult) => void }) {
  ...
  return (
    <Modal className={`topup-modal ${result ? "topup-modal--success" : ""}`} open={open} title={result ? "Recarga exitosa" : "Recargar saldo demo"} description={result ? "El XLM ya llegó a tu wallet de Stellar Testnet." : "Recibe XLM de prueba y el crédito equivalente para realizar aportes."} onClose={close} dismissible={!busy}>
      {result ? (
